#!/usr/bin/env python3
"""
Requests + ThreadPool Layer-7 flood generator (LAB USE ONLY)

Why this exists:
- A synchronous baseline engine using requests (threads) to compare vs AsyncIO/aiohttp
- Useful for methodology comparisons: Async vs threaded sync clients

Writes results to --results-dir (default ./results).
"""

import argparse
import json
import os
import random
import sys
import time
from concurrent.futures import ThreadPoolExecutor, as_completed
from datetime import datetime, timezone
from typing import Dict, List, Optional

import requests


import aiohttp
from pathlib import Path

REPORTS_ROOT = Path.home() / "ADL-TEST-GUARD" / "reports-area" / "python-l7-flood" / "requests"
REPORTS_ROOT.mkdir(parents=True, exist_ok=True)




def now_iso_utc() -> str:
    return datetime.now(timezone.utc).isoformat()


def utc_stamp() -> str:
    return datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")


def ensure_dir(path: str) -> None:
    os.makedirs(path, exist_ok=True)


def read_payload(path: Optional[str], size_bytes: int) -> str:
    if path:
        with open(path, "r", encoding="utf-8") as f:
            return f.read()
    if size_bytes <= 0:
        return ""
    base = {"ts": now_iso_utc(), "data": ""}
    overhead = len(json.dumps(base).encode("utf-8")) + 20
    fill_len = max(0, size_bytes - overhead)
    base["data"] = "A" * fill_len
    return json.dumps(base)


def do_one(
    wid: int,
    method: str,
    url: str,
    headers: Dict[str, str],
    body: Optional[str],
    timeout_s: float,
) -> Dict:
    t0 = time.perf_counter()
    try:
        resp = requests.request(
            method=method,
            url=url,
            headers=headers,
            data=body if method in ("POST", "PUT", "PATCH") else None,
            timeout=timeout_s,
        )
        # consume content
        _ = resp.content
        latency_ms = (time.perf_counter() - t0) * 1000.0
        return {
            "ts": now_iso_utc(),
            "worker": wid,
            "method": method,
            "url": url,
            "ok": True,
            "status": int(resp.status_code),
            "latency_ms": round(latency_ms, 3),
            "error": "",
        }
    except Exception as e:
        latency_ms = (time.perf_counter() - t0) * 1000.0
        return {
            "ts": now_iso_utc(),
            "worker": wid,
            "method": method,
            "url": url,
            "ok": False,
            "status": 0,
            "latency_ms": round(latency_ms, 3),
            "error": repr(e),
        }


def summarize(jsonl_path: str) -> Dict:
    total = 0
    ok = 0
    status_counts = {}
    latencies = []

    with open(jsonl_path, "r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
            if not line:
                continue
            total += 1
            rec = json.loads(line)
            if rec.get("ok"):
                ok += 1
                st = int(rec.get("status", 0))
                status_counts[st] = status_counts.get(st, 0) + 1
            latencies.append(float(rec.get("latency_ms", 0.0)))

    latencies.sort()

    def pct(p: float) -> float:
        if not latencies:
            return 0.0
        idx = int(round((p / 100.0) * (len(latencies) - 1)))
        return float(latencies[max(0, min(idx, len(latencies) - 1))])

    return {
        "total_requests": total,
        "ok_requests": ok,
        "error_requests": total - ok,
        "ok_rate": (ok / total) if total else 0.0,
        "status_counts": dict(sorted(status_counts.items(), key=lambda x: x[0])),
        "latency_ms": {
            "min": latencies[0] if latencies else 0.0,
            "p50": pct(50),
            "p90": pct(90),
            "p95": pct(95),
            "p99": pct(99),
            "max": latencies[-1] if latencies else 0.0,
        },
    }


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Requests (threaded) L7 flood generator (LAB USE ONLY)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--url", action="append", required=True,
                   help="Target URL. Use multiple times for a pool.")
    p.add_argument("--method", default="GET", choices=["GET", "POST", "PUT", "PATCH"])
    p.add_argument("--threads", type=int, default=50,
                   help="Number of worker threads.")
    p.add_argument("--duration", type=int, default=30,
                   help="Duration in seconds.")
    p.add_argument("--rps", type=float, default=0.0,
                   help="Approx global RPS cap (0 disables).")
    p.add_argument("--timeout", type=float, default=10.0,
                   help="Request timeout seconds.")
    p.add_argument("--scenario", default="l7_flood_requests",
                   help="Value for X-Test-Scenario header.")
    p.add_argument("--user-agent", default="adl-requests-l7-flood",
                   help="User-Agent header.")
    p.add_argument("--header", action="append",
                   help="Extra header in 'Key: Value' format. Can be repeated.")
    p.add_argument("--payload-size", type=int, default=0,
                   help="Payload size in bytes (best-effort) for POST/PUT/PATCH if no file.")
    p.add_argument("--payload-file", default=None,
                   help="Path to payload file for POST/PUT/PATCH.")

    p.add_argument("--results-dir", default=str(REPORTS_ROOT),
                   help="Where to write output files (JSONL + summary).")



#    p.add_argument("--results-dir", default="./results",
#                   help="Where to write output files (JSONL + summary).")
    p.add_argument("--run-id", default=None,
                   help="Override run id filename prefix.")
    return p.parse_args()


def main() -> int:
    args = parse_args()

#    results_dir = os.path.abspath(args.results_dir)
    

    results_dir = str(REPORTS_ROOT.resolve())

    ensure_dir(results_dir)
    run_id = args.run_id or f"requests_l7_{utc_stamp()}"
    out_jsonl = os.path.join(results_dir, f"{run_id}.jsonl")
    out_summary = os.path.join(results_dir, f"{run_id}.summary.json")

    urls: List[str] = [u.strip() for u in args.url if u.strip()]
    if not urls:
        print("❌ No URLs provided.", file=sys.stderr)
        return 2

    method = args.method.upper()

    headers: Dict[str, str] = {
        "User-Agent": args.user_agent,
        "X-Test-Scenario": args.scenario,
    }
    for hv in args.header or []:
        if ":" in hv:
            k, v = hv.split(":", 1)
            headers[k.strip()] = v.strip()

    body = None
    if method in ("POST", "PUT", "PATCH"):
        body = read_payload(args.payload_file, args.payload_size)

    print(f"▶ run_id:      {run_id}")
    print(f"▶ urls:        {len(urls)}")
    print(f"▶ method:      {method}")
    print(f"▶ threads:     {args.threads}")
    print(f"▶ duration_s:  {args.duration}")
    print(f"▶ rps_cap:     {args.rps if args.rps > 0 else 'disabled'}")
    print(f"▶ scenario:    {args.scenario}")
    print(f"▶ results_dir: {results_dir}")
    print(f"▶ out_jsonl:   {out_jsonl}")

    end_time = time.perf_counter() + args.duration
    rng = random.Random(int(time.time()))
    per_thread_delay = (args.threads / args.rps) if args.rps > 0 else 0.0  # rough pacing

    with open(out_jsonl, "w", encoding="utf-8") as f, ThreadPoolExecutor(max_workers=args.threads) as ex:
        futures = []
        wid = 0

        while time.perf_counter() < end_time:
            # schedule one request per loop iteration
            wid = (wid % args.threads) + 1
            url = rng.choice(urls)
            futures.append(ex.submit(do_one, wid, method, url, headers, body, args.timeout))

            # rough global rate control
            if args.rps > 0 and per_thread_delay > 0:
                time.sleep(max(0.0, per_thread_delay / args.threads))

        for fut in as_completed(futures):
            rec = fut.result()
            f.write(json.dumps(rec) + "\n")

    s = summarize(out_jsonl)
    with open(out_summary, "w", encoding="utf-8") as f:
        json.dump({"run_id": run_id, "ended_utc": now_iso_utc(), "summary": s}, f, indent=2)

    print("✅ Done.")
    print(f"📄 Summary: {out_summary}")
    print(json.dumps(s, indent=2))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
